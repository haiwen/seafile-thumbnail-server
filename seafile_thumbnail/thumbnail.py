...
from seafile_thumbnail.constants import VIDEO, PDF, XMIND, SVG, SEADOC, RAW, EXR, EPS
...
def generate_thumbnail(request, thumbnail_info):
    ...
    if filetype == EPS:
        task_id, status = thumbnail_task_manager.add_pdf_or_psd_create_task(create_eps_thumbnail, repo_id, file_id, path,
                                                             size, thumbnail_file, file_size)
        if status != 200:
            return (task_id, status)
        return (task_id, 200)
...
def create_exr_thumbnail(repo_id, file_id, path, size, thumbnail_file, file_size):
    """Create EXR thumbnail using OpenCV.
    
    EXR files contain HDR (High Dynamic Range) data, so need to apply
    tone mapping to convert to displayable 8-bit image.
    OpenCV has native support for EXR format.
    """
    try:
        # Enable OpenEXR support in OpenCV (disabled by default for security)
        os.environ['OPENCV_IO_ENABLE_OPENEXR'] = '1'
        import cv2
        import numpy as np
    except ImportError:
        logger.error("Could not find opencv-python installed. "
                     "Please install by 'pip install opencv-python-headless'")
        raise Exception("Missing dependency: opencv-python-headless")
    ...
    try:
        ...
        # Read EXR with OpenCV (returns float32 BGR data)
        hdr_data = cv2.imread(tmp_exr_path, cv2.IMREAD_ANYCOLOR | cv2.IMREAD_ANYDEPTH)
        
        if hdr_data is None:
            raise Exception(f"Failed to read EXR file: {path}")
...
def create_eps_thumbnail(repo_id, file_id, path, size, thumbnail_file, file_size):
    """Create EPS (Encapsulated PostScript) thumbnail using Ghostscript.
    
    Ghostscript is used to convert EPS to PNG, as pdftoppm does not support EPS format.
    """
    tmp_eps_path = os.path.join(tempfile.gettempdir(), f"{file_id}.eps")
    tmp_png_path = os.path.join(tempfile.gettempdir(), f"{file_id}.png")
    
    try:
        t1 = timeit.default_timer()
        
        # Stream EPS file to temp path
        stream_file_to_path(repo_id, file_id, tmp_eps_path)
        
        # Use Ghostscript to convert EPS to PNG
        # -dSAFER: run in safer mode
        # -dBATCH: exit after processing
        # -dNOPAUSE: don't pause between pages
        # -sDEVICE=png16m: output as PNG with 24-bit color
        ...
        try:
            subprocess.check_output(command, stderr=subprocess.STDOUT, timeout=60)
        except subprocess.TimeoutExpired:
            logger.error('EPS thumbnail generation timed out after 60 seconds')
            raise Exception('EPS processing timeout')
        except subprocess.CalledProcessError as e:
            logger.error(f'Ghostscript failed: {e.output.decode() if e.output else e}')
            raise
        
        t2 = timeit.default_timer()
        logger.debug(f"Convert EPS [{path}] to PNG takes: {t2 - t1:.2f}s (size: {file_size} bytes)")
        
        _create_thumbnail_common(tmp_png_path, thumbnail_file, size)
    except Exception as e:
        logger.exception(e)
        logger.error(f"Failed to generate EPS thumbnail for [{path}]: {str(e)}")
        raise e
    finally:
        if os.path.exists(tmp_eps_path):
            os.unlink(tmp_eps_path)
        if os.path.exists(tmp_png_path):
            os.unlink(tmp_png_path)
    return
